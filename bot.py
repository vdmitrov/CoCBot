import logging
import requests
import asyncio
from aiogram import Bot, Dispatcher, types

# Настройка логирования
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")

# Токены (замени на актуальные)
TELEGRAM_BOT_TOKEN = "7930052685:AAHh_3Cl86M3p6u70BThTCbE6rDQKOiDt8s"
COC_API_TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9..Bfs3nh0cnfXxAfHzNIEbTCIiimhoAvq1IgylqcBMV2X07bBsV7FvzmLhttvpBZwOsDkt4uzrxVC8xTxii9prNA"
CLAN_TAG = "#2GCG2C0YP"

# Создание бота
bot = Bot(token=TELEGRAM_BOT_TOKEN)
dp = Dispatcher(bot)

# Функция для получения информации о клане
def get_clan_data():
    url = f"https://api.clashofclans.com/v1/clans/{CLAN_TAG.replace('#', '%23')}"
    headers = {"Authorization": f"Bearer {COC_API_TOKEN}"}
    
    try:
        response = requests.get(url, headers=headers)
        logging.info(f"Запрос к API: {url}, Статус-код: {response.status_code}")

        if response.status_code == 200:
            data = response.json()
            logging.info(f"Данные получены: {data}")
            return data
        else:
            logging.error(f"Ошибка API: {response.status_code} - {response.text}")
            return None
    except Exception as e:
        logging.exception("Ошибка при запросе к API")
        return None

# Проверка доступности API перед запуском
if not get_clan_data():
    logging.critical("API Clash of Clans недоступен! Проверь токен.")
    exit(1)

# Обработчик команды /start
@dp.message_handler(commands=["start"])
async def send_welcome(message: types.Message):
    logging.info(f"Пользователь {message.from_user.id} запустил бота")
    await message.reply("Привет! Используй /info, чтобы получить информацию о клане.")

# Обработчик команды /info
@dp.message_handler(commands=["info"])
async def send_clan_info(message: types.Message):
    logging.info(f"Пользователь {message.from_user.id} запросил /info")

    data = get_clan_data()
    if data:
        clan_name = data.get("name", "Неизвестно")
        members = data.get("memberList", [])

        player_list = []
        for player in members:
            name = player.get("name", "Без имени")
            role = player.get("role", "member")
            level = player.get("expLevel", "?")
            townhall = player.get("townHallLevel", "?")
            trophies = player.get("trophies", "?")

            role_emoji = {
                "leader": "👑",
                "coLeader": "⭐",
                "elder": "🛡",
                "member": "🔹"
            }.get(role, "❓")

            player_list.append(f"{role_emoji} {name} (⭐ {level}, 🏠 {townhall}, 🏆 {trophies})")

        text = f"🏰 **Клан {clan_name}**\n👥 Участники: {len(members)}\n\n" + "\n".join(player_list)
        await message.reply(text, parse_mode="Markdown")
    else:
        logging.error("Ошибка получения данных о клане")
        await message.reply("Ошибка: не удалось получить данные о клане 😢")

# Запуск бота
async def main():
    logging.info("Бот запущен и ожидает команды")
    await dp.start_polling()

if __name__ == "__main__":
    asyncio.run(main())
